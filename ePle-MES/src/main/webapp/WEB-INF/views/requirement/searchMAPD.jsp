...
<body>
			...
			<table class="table table-striped" id="tableId">
				...
				<tbody>
					<c:forEach items="${list}" var="item">
						<tr onclick="selectWork('${item.code}','${item.name}')">
							<th>${item.code}</th>
							<th>${item.name}</th>
						</tr>
					</c:forEach>
	...
	<script type="text/javascript">
		...
		function selectWork(a, b) { // 부모창으로 값 넘기기
			...
			opener.document.getElementById("mapd_code").value = a //출하번호
			opener.document.getElementById("mapdName").value = b //회사명

			opener.document.getElementById("mapd_code").dispatchEvent(
					new Event('input'));
			opener.document.getElementById("mapdName").dispatchEvent(
					new Event('input'));
